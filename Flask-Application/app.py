...
ARBITRUM_RPC_URL = f"https://arbitrum-sepolia.infura.io/v3/{INFURA_KEY}"
arb_web3 = Web3(Web3.HTTPProvider(ARBITRUM_RPC_URL))
arb_admin = arb_web3.eth.account.from_key(private_key)
arb_web3.eth.default_account = arb_admin.address

AVALANCHE_RPC_URL = f"https://avalanche-fuji.infura.io/v3/{INFURA_KEY}"
avax_web3 = Web3(Web3.HTTPProvider(AVALANCHE_RPC_URL))
avax_admin = avax_web3.eth.account.from_key(private_key)
avax_web3.eth.default_account = avax_admin.address
...
def gamedev_signup():
    ...
    account = avax_web3.eth.account.create()
    wallet_address = account.address
    private_key = account.key.hex()
    value = avax_web3.to_wei(0.01, "ether")
    gas_price = avax_web3.eth.gas_price
    ...
    transaction = {
        ...
        "nonce": avax_web3.eth.get_transaction_count(avax_admin.address),
        "chainId": avax_web3.eth.chain_id,
    }
    # Sign and send transaction
    signed_txn = avax_web3.eth.account.sign_transaction(transaction, avax_admin.key.hex())
    tx_hash = avax_web3.eth.send_raw_transaction(signed_txn.raw_transaction)
    ...
    transaction = {
        ...
        "nonce": arb_web3.eth.get_transaction_count(arb_admin.address),
        "chainId": arb_web3.eth.chain_id,
    }
    # Sign and send transaction
    signed_txn = arb_web3.eth.account.sign_transaction(transaction, arb_admin.key.hex())
    tx_hash = arb_web3.eth.send_raw_transaction(signed_txn.raw_transaction)
...
def eth_balance_tool(wallet_address: str) -> str:
    ...
    """
    try:
        balance_wei = avax_web3.eth.get_balance(wallet_address)
        balance_eth = avax_web3.from_wei(balance_wei, "ether")
...
def pay_game_fee_tool(
    ...
    value = avax_web3.to_wei(amount, "ether")
    gas_price = avax_web3.eth.gas_price
    ...
    transaction = {
        ...
        "gas": 21000,
        "gasPrice": gas_price,
        "nonce": avax_web3.eth.get_transaction_count(myacc.address),
        "chainId": avax_web3.eth.chain_id,
    }

    # Sign and send the transaction.
    signed_txn = avax_web3.eth.account.sign_transaction(transaction, myacc.key.hex())
    tx_hash = avax_web3.eth.send_raw_transaction(signed_txn.raw_transaction)
    ...
    return json.dumps(
        {"message": "Game fee paid", "transaction_hash": avax_web3.to_hex(tx_hash)}
    )
...
def create_token():
    ...
    if gamedev == {}:
        return jsonify({"error": "Invalid email or password"}), 401
    if network=="avax":
        account = avax_web3.eth.account.from_key(gamedev["Private Key"])
        ERC20 = avax_web3.eth.contract(
        ...
        transaction = ERC20.constructor(
        ...
        ).build_transaction(
            {
                "from": account.address,
                "gas": 1000000,
                "gasPrice": avax_web3.to_wei("1", "gwei"),
                "nonce": avax_web3.eth.get_transaction_count(account.address),
            }
        )

        # Sign and send
        signed_txn = avax_web3.eth.account.sign_transaction(transaction, account.key.hex())
        tx_hash = avax_web3.eth.send_raw_transaction(signed_txn.raw_transaction)
        tx_receipt = avax_web3.eth.wait_for_transaction_receipt(tx_hash)

        contract_address = tx_receipt.contractAddress
    elif network=="arbitrum":
        account = arb_web3.eth.account.from_key(gamedev["Private Key"])
        ERC20 = arb_web3.eth.contract(
            abi=contract_interface["abi"], bytecode=contract_interface["bin"]
        )
        ...
        transaction = ERC20.constructor(
        ...
        ).build_transaction(
            {
                "from": account.address,
                "gas": 1000000,
                "gasPrice": arb_web3.to_wei("1", "gwei"),
                "nonce": arb_web3.eth.get_transaction_count(account.address),
            }
        )

        # Sign and send
        signed_txn = arb_web3.eth.account.sign_transaction(transaction, account.key.hex())
        tx_hash = arb_web3.eth.send_raw_transaction(signed_txn.raw_transaction)
        tx_receipt = arb_web3.eth.wait_for_transaction_receipt(tx_hash)
...
def signup():
    data = request.json
    email = data["email"]
    metamask_wallet = data["metamask_wallet"]
    ...
    value = arb_web3.to_wei(0.0001, "ether")
    transaction = {
        "to": wallet_address,
        "value": value,
        "gas": 21000,
    ...
    success = utils.upload_user(email, metamask_wallet, password, wallet_address, private_key)
...
def get_profile_info(user_id):
    ...
    print(str(user["Metamask Wallet"]))
    user_profile = {
        "email": user["Email"],
        "wallet": str(user["Metamask Wallet"]),
...
def withdraw_tokens():
    ...
    wallet_address = str(user["Metamask Wallet"])

    transaction_hashes = []
    utils.token_gen.update_config()
    token=utils.get_token_by_contract_address(token_address)
    if token["network"]=="avax":
        web3=avax_web3
    else:
        web3=arb_web3
    ...
    if myacc:
        ...
        transaction = contract.functions.transfer(
            web3.to_checksum_address(wallet_address), value
        ).build_transaction(
            {
                "from": myacc.address,
                "gas": 100000,
            ...
            jsonify(
                {
                    ...
                    "transaction_hash": str(avax_web3.to_hex(tx_hash)),
                }
            ),
            200,
        )
...
def withdraw_eth():
    ...
    return (
        jsonify({"message": "ETH withdrawn", "transaction_hash": avax_web3.to_hex(tx_hash)}),
        200,
    )
